...
class Kind::OptionalTest < Minitest::Test
  def test_optional_value
    ...
    optional2 = Kind::Optional.new(nil)

    assert_nil(optional2.value)

    # ---

    optional3 = Kind::Optional.new(Kind::Undefined)
  ...
  def test_optional_when_some
    assert_predicate(Kind::Optional.new(2), :some?)

    refute_predicate(Kind::Optional.new(nil), :some?)
    refute_predicate(Kind::Optional.new(Kind::Undefined), :some?)
  end

  def test_optional_when_none
    assert_predicate(Kind::Optional.new(nil), :none?)
    assert_predicate(Kind::Optional.new(Kind::Undefined), :none?)

    refute_predicate(Kind::Optional.new(1), :none?)
  end

  def test_optional_map_when_none
    optional1 = Kind::Optional.new(2)
  ...
  def test_optional_when_map_returns_nil
    optional1 = Kind::Optional.new(2)
  ...
  def test_optional_when_map_returns_undefined
    optional1 = Kind::Optional.new(3)
  ...
  def test_optional_constructor_alias
    assert_instance_of(Kind::Optional, Kind::Optional[1])
  end
  ...
  def test_try_method
    assert_equal('FOO', Kind::Optional['foo'].try(:upcase))
    assert_equal('FOO', Kind::Optional['foo'].try { |value| value.upcase })

    assert_nil(Kind::Optional[nil].try(:upcase))
    assert_nil(Kind::Optional[nil].try { |value| value.upcase })

    assert_nil(Kind::Optional[Kind::Undefined].try(:upcase))
    assert_nil(Kind::Optional[Kind::Undefined].try { |value| value.upcase })

    assert_raises_with_message(Kind::Error, '"upcase" expected to be a kind of Symbol') do
      Kind::Optional['foo'].try('upcase')
    end
  end
end
